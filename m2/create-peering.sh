...
aws cloudformation create-stack --stack-name Globomantics2 --template-body file://basic-network-2.json

# Once the VPC is in place, we'll create the peering connections
# We'll need some information

vpc1_id=$(aws ec2 describe-vpcs --filters Name="tag:Name",Values="globo-primary" \
  --query 'Vpcs[0].VpcId' --output text)

vpc2_id=$(aws ec2 describe-vpcs --filters Name="tag:Name",Values="globo-secondary" \
  --query 'Vpcs[0].VpcId' --output text)
